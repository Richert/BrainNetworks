...
class QIFMicro(object):
    ...
    def run(self, T, dt, I=None, dts=None, **kwargs):
        ...
        for step in range(steps_sim):
            ...
            wait, mask, spike = self.calculate_rate(wait, wait_min, spike_time, dt)

            # update state variables
            self.update(dt, mask, I[step])

            # calculate new spikes
            wait, spike_time = self.elicit_spikes(wait, wait_min, spike_time, dt)
    ...
    def elicit_spikes(self, wait, wait_min, spike_time, dt):
        ...
        return wait, spike_time

    def calculate_rate(self, wait, wait_min, spike_time, dt):
        ...
        return wait, mask, spike


class QIFMacro(object):

    def __init__(self, J=15.0, eta_mean=-5.0, eta_fwhm=1.0, tau=0.02, alpha=0.0, tau_a=0.1, v_0=-2.0, r_0=0.0):
    ...
    def update(self, dt, I=0.0):
        ...
        v_delta = (self.v**2 + self.eta + I) / self.tau \
                  + self.J*(1.0 - np.arctan(self.e))*self.r \
