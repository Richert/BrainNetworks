    ...
    def eval_fitness(self, target: list, **kwargs):
        ...
        if len(param_grid) > 0:
            ...
            res_file = self.cgs.run(
                ...
                worker_kwargs={'freq_targets': freq_targets, 'y': target, 'time_lim': 1000.0, 'cpu_lim': True,
...
if __name__ == "__main__":
    ...
    compute_dir = f"{os.getcwd()}/gpe_2pop_opt"

    ga = CustomGOA(fitness_measure=fitness,
                   gs_config={
                       'circuit_template': f"{os.getcwd()}/config/stn_gpe/gpe_2pop",
                       'permute_grid': True,
                       'param_map': param_map,
                       'simulation_time': T,
                       'step_size': dt,
                       ...
                       'inputs': {},
                       'outputs': {'r_i': 'gpe_p/gpe_proto_syns_op/R_i', 'r_a': 'gpe_a/gpe_arky_syns_op/R_a'},
    ...
    winner = ga.run(
        ...
        target=[[62, 31],  # healthy control
                [35, np.nan],  # ampa blockade in GPe
                [76, np.nan],  # ampa and gabaa blockade in GPe
                [135, np.nan],  # GABAA blockade in GPe
                ],
        max_iter=500,
